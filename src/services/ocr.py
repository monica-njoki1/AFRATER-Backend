...
Claude Vision temporarily disabled — no API credits.
Uses basic image text extraction instead.
Re-enable by removing the early return when credits are available.
"""
...
def extract_text_from_screenshot(filepath: str) -> dict:
    """
    Screenshot analysis — Claude Vision temporarily disabled.
    Returns a message asking user to paste text manually instead.
    """
    # ── Re-enable this block when Anthropic credits are available ──
    # import anthropic, json
    # client = anthropic.Anthropic(api_key=os.environ.get("ANTHROPIC_API_KEY"))
    # ... rest of Claude Vision code ...
    ...
    return {
        ...
        "is_mpesa":  False,
        "disabled":  True,
        "note":      "Screenshot AI analysis is temporarily unavailable. Please copy the message text and use the Check Message tab instead.",
    }